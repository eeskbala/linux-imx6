...
static s8 gtp_i2c_test(struct i2c_client *client, int num_tries);
...
#if GTP_DEBUG_ON
                s32 retval = 0;
                u8  dbuff[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH] = {0};

                dbuff[0] = config[0];
                dbuff[1] = config[1];

                retval = gtp_i2c_read(client, dbuff, (GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH));
                GTP_INFO("Read data back from device returns %d",retval);

                if (retval > 0) {
                        for (k = 0; k < (GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH); k++) {
                                GTP_INFO("DATA DUMP Loc: %02X : %02X", k, dbuff[k]);
                        }
                }
#endif
                break;
...
/*******************************************************
Function:
    Get gpio(INT & RST) ports from device tree
Input:
    ts: private data.
Output:
    Executive outcomes.
        >= 0: succeed, < 0: failed
*******************************************************/
static s8 gtp_setup_io_port(struct goodix_ts_data *ts, struct device *dev)
{
    int ret;
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    if (ret < 0) 
    {
        GTP_ERROR("Failed to request IRQ GPIO:%d, ERRNO:%d", (s32)GTP_INT_PORT, ret);
        ret = -ENODEV;
    }
    else
    {
    ...
    if (ret < 0) 
    {
        GTP_ERROR("Failed to request RESET GPIO:%d, ERRNO:%d",(s32)GTP_RST_PORT,ret);
        ret = -ENODEV;
    }
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (strcmp(ts_select, "gt9xx")) {
            printk("*** GT9XX not enabled on command line\n");
            return -ENODEV;
    }
#endif
    ...
    ret = gtp_setup_io_port(ts, dev);
    if (ret < 0)
    {
        GTP_ERROR("GTP setup IO port failed.");
        kfree(ts);
        return ret;
    }
