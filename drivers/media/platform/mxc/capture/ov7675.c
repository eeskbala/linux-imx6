 ...
 */
	...
	// Vsync positive
	{ 0xa2, 0x02 },
#if defined(CONFIG_MXC_CAMERA_OV7675_INVERT_PCLK)
        { REG_COM10, (COM10_PCLK_REV|COM10_HREF_REV) },
#else
        { REG_COM10, (COM10_HREF_REV) },
#endif
...
static int ov7675_init(struct i2c_client *client)
{
	...
	int unbanding_mode, night_mode, colorfx;
        ...
        // Only reset the sensor, etc, once
        if (!first)
                return 0;
        first = 0;
...
static int ov7675_detect(struct i2c_client *client)
{
	...
	v4l_info(client, "OmniVision %s camera found\n", client->name);
...
#if 1	// These formats (from the OV7670) do not seem to work for the OV7675 (even if at least QVGA is supposed to be supported).
	/* CIF */
	{
		.width		= CIF_WIDTH,
		.height		= CIF_HEIGHT,
...
static int ov7675_t_unbanding(struct i2c_client *client, int value)
{
	...
	if (value == V4L2_CID_POWER_LINE_FREQUENCY) {
		ret = ov7675_read(client, REG_COM8, &v);
		msleep(10);  /* FIXME */
...
static int ov7675_t_night_mode(struct i2c_client *client, int value)
{
	...
	if (value) {
		...
		ret += ov7675_write(client, REG_CLKRC, REG_CLKRC_DEFAULT);
...
//		pr_err("func %s, file %s, line %d\n", __FUNCTION__, __FILE__, __LINE__);
//		return -EINVAL;
//	}

	/* Do we already reached all discrete framesizes? */
	if (frms->index >= N_WIN_SIZES) {
                // Note: this is common when running 'yavta --enum-formats'
//		pr_err("func %s, file %s, line %d\n", __FUNCTION__, __FILE__, __LINE__);
		return -EINVAL;
	}

	frms->pixel_format = ov7675_formats[frms->index].pixelformat;
...
static int ioctl_enum_fmt_cap(struct v4l2_int_device *s, struct v4l2_fmtdesc *fmt)
{
	...
	return ov7675_enum_fmt(sensor->i2c_client, fmt);
}
...
static int ioctl_try_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	return ov7675_try_fmt(sensor->i2c_client, f, NULL, NULL);
}
...
static int ioctl_s_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	return ov7675_s_fmt(sensor->i2c_client, f);
}
...
static int ioctl_g_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	return ov7675_g_parm(sensor->i2c_client, a);
}
...
static int ioctl_s_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	return ov7675_s_parm(sensor->i2c_client, a);
}
...
static int ioctl_queryctrl(struct v4l2_int_device *s, struct v4l2_queryctrl *qctrl)
{
	...
	return ov7675_queryctrl(sensor->i2c_client, qctrl);
}
...
static int ioctl_g_ctrl(struct v4l2_int_device *s, struct v4l2_control *ctrl)
{
	...
	return ov7675_g_ctrl(sensor->i2c_client, ctrl);
}
...
static int ioctl_s_ctrl(struct v4l2_int_device *s, struct v4l2_control *ctrl)
{
	...
	return ov7675_s_ctrl(sensor->i2c_client, ctrl);
}
...
static int ioctl_g_ifparm(struct v4l2_int_device *s, struct v4l2_ifparm *p)
{
	...
	if (p == NULL)
        ...
        p->u.bt656.clock_min = OV7675_CLOCK_MIN;
        p->u.bt656.clock_max = OV7675_CLOCK_MAX;
...
static enum ov7675_power_state ov7675_get_power(struct v4l2_int_device *s, enum ov7675_power_client client)
{
        struct ov7675_decoder *decoder = &ov7675_dev;
...
static int ov7675_set_power(struct v4l2_int_device *s, enum ov7675_power_state on, enum ov7675_power_client client)
{
	...
	case OV7675_CLIENT_SYSFS:
		...
		if (on == OV7675_POWER_ON && decoder->v4l2_power_state == OV7675_POWER_OFF)
			ov7675_t_night_mode(sensor->i2c_client, 1);
		break;
	case OV7675_CLIENT_V4L2:
	...
	*client_power_state = on;
	...
	case OV7675_POWER_OFF:
		if ((decoder->sysfs_power_state == OV7675_POWER_OFF &&
		    ...
		    decoder->pwdn) {
                        gpio_set_value_cansleep(decoder->pwdn, OV7675_POWER_OFF);
			decoder->state = STATE_NOT_DETECTED;
		}
	break;
	...
	case OV7675_POWER_ON:
		if ((decoder->sysfs_power_state == OV7675_POWER_ON ||
		    ...
		    decoder->state == STATE_NOT_DETECTED &&
		    decoder->pwdn) {
                        gpio_set_value_cansleep(decoder->pwdn, OV7675_POWER_ON);
			/* Wait a little before any I2C access (OV datasheet says 1 ms) */
			msleep(10);

			/* Detect the sensor */
			err |= ov7675_detect(sensor->i2c_client);
			if (err) {
				v4l_err(sensor->i2c_client,
			...
			err |= ov7675_init(sensor->i2c_client);
...
static int ioctl_init(struct v4l2_int_device *s)
{
	...
	return ov7675_init(sensor->i2c_client);
}
...
static int ioctl_reset(struct v4l2_int_device *s)
{
	...
	ov7675_reset(sensor->i2c_client);
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	tgt_xclk = min(tgt_xclk, (u32)OV7675_CLOCK_MAX);
	tgt_xclk = max(tgt_xclk, (u32)OV7675_CLOCK_MIN);
        sensor->mclk = tgt_xclk;

	pr_err("   Setting mclk to %d MHz\n", tgt_xclk / 1000000);
	clk_set_rate(sensor->sensor_clk, sensor->mclk);

	err = ov7675_detect(sensor->i2c_client);
	if (err < 0) {
		v4l_err(sensor->i2c_client, "Unable to detect decoder\n");
		return err;
	}
...
static int ioctl_g_chip_ident(struct v4l2_int_device *s, int *id)
{
	...
	strcpy(((struct v4l2_dbg_chip_ident *)id)->match.name, "ov7675_camera");
...
static struct ov7675_decoder ov7675_dev = {
	...
	.current_unbanding_mode = V4L2_CID_POWER_LINE_FREQUENCY_60HZ,
...
static int ov7675_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	i2c_set_clientdata(client, decoder);
        ...
        // Incestuous setup
	memset(&ov7675_sensor_data, 0, sizeof(ov7675_sensor_data));
	ov7675_sensor_data.sensor_clk = devm_clk_get(dev, "csi_mclk");
	err = of_property_read_u32(dev->of_node, "mclk", &val);
	if (err) {
		dev_err(dev, "mclk missing or invalid\n");
		return -ENODEV;
	}
        ov7675_sensor_data.mclk = val;
	err = of_property_read_u32(dev->of_node, "mclk_source", &val);
	if (err) {
		dev_err(dev, "mclk_source missing or invalid\n");
		return -ENODEV;
	}
        ov7675_sensor_data.mclk_source = val;
	err = of_property_read_u32(dev->of_node, "ipu_id", &val);
	if (err) {
		dev_err(dev, "ipu_id missing or invalid\n");
		return -ENODEV;
	}
        ov7675_sensor_data.ipu_id = val;
	err = of_property_read_u32(dev->of_node, "csi_id", &val);
	if (err) {
		dev_err(dev, "csi_id missing or invalid\n");
		return -ENODEV;
	}
        ov7675_sensor_data.csi = val;

	ov7675_sensor_data.i2c_client = client;
	ov7675_sensor_data.pix.pixelformat = V4L2_PIX_FMT_YUYV;
	ov7675_sensor_data.pix.width = 640;
	ov7675_sensor_data.pix.height = 480;
	ov7675_sensor_data.streamcap.capability = V4L2_MODE_HIGHQUALITY |
                                                  V4L2_CAP_TIMEPERFRAME;
	ov7675_sensor_data.streamcap.capturemode = 0;
	ov7675_sensor_data.streamcap.timeperframe.denominator = 30;
	ov7675_sensor_data.streamcap.timeperframe.numerator = 1;
	ov7675_int_device.priv = &ov7675_sensor_data;

#if 0  // FIXME if this is ever needed
	...
	if (ov7675_detect(decoder->client) < 0) {
                pr_err("OV7675 not detected\n");
                return -ENODEV;
        }
