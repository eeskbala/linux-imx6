...
static int prp_enc_setup(cam_data *cam)
{
	...
	case IPU_CSI_CLK_MODE_CCIR1120_PROGRESSIVE_SDR:
		enc.csi_prp_enc_mem.interlaced = false;
		break;
	case IPU_CSI_CLK_MODE_CCIR656_INTERLACED:
	case IPU_CSI_CLK_MODE_CCIR1120_INTERLACED_DDR:
	case IPU_CSI_CLK_MODE_CCIR1120_INTERLACED_SDR:
		enc.csi_prp_enc_mem.interlaced = true;
		break;
	default:
	...
	pr_debug("%s:in:%dx%d out:%dx%d format=%x %sinterlaced\n", __func__,
			...
			enc.csi_prp_enc_mem.out_width, enc.csi_prp_enc_mem.out_height,
                        ...
                        enc.csi_prp_enc_mem.interlaced ? "" : "non-"
                );
